...
/* Database Transactional Log Backup */
CREATE PROCEDURE [dbo].[internalDataArchiveTrans] (
...
If(Right(@destination, 1) != '\')
	Set @instancedest = @destination + '\' + @@SERVERNAME + '\TLog'
Else
...
SET @pattern = N'%[_]db[_]20[0-9][0-9][0-1][0-9][0-3][0-9][0-2][0-9][0-5][0-9].trn'
...
BEGIN
	...
	BEGIN
		...
		set @cmd = 'EXEC master.dbo.xp_cmdshell ''dir /b "' + @instancedest + '\' + @dbname +'\' + @dbname +'_db_"*.trn'''
--		PRINT @cmd
		...
		IF @version >= 10.5
			Set @cmd = 'BACKUP LOG [' + @dbname +'] TO DISK = ''' + @instancedest + '\' + @dbname + '\' + @dbname + '_db_' + @timestamp + '.trn'' WITH INIT,COMPRESSION'
		ELSE
			Set @cmd = 'BACKUP LOG [' + @dbname +'] TO DISK = ''' + @instancedest + '\' + @dbname + '\' + @dbname + '_db_' + @timestamp + '.trn'' WITH INIT'

		--PRINT (@cmd)
		EXEC(@cmd)

		Set @cmd = 'RESTORE VERIFYONLY FROM DISK = ''' + @instancedest + '\' + @dbname + '\' + @dbname + '_db_' + @timestamp + '.trn'''
		EXEC (@cmd)
